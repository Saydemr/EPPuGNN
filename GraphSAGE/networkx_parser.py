...
np_adj_matrix = np.zeros((27213,27213), dtype=np.int8)
...
with open("BIOGRID-ORGANISM-Homo_sapiens-4.4.204.tab3.txt") as f:
    ...
    for line in f:
        ...
        if int(line[1]) not in id_map_int and int(line[2]) not in id_map_int:
            ...
            np_adj_matrix[i][i+1] = 1
            np_adj_matrix[i+1][i] = 1
            #print(i, i+1)
            i+=2
            ...
            np_adj_matrix[id_map_int[int(line[1])]][i] = 1
            np_adj_matrix[i][id_map_int[int(line[1])]] = 1
            ...
            np_adj_matrix[i][id_map_int[int(line[2])]] = 1
            np_adj_matrix[id_map_int[int(line[2])]][i] = 1     
            ...
            np_adj_matrix[id_map_int[int(line[1])]][id_map_int[int(line[2])]] = 1
            np_adj_matrix[id_map_int[int(line[2])]][id_map_int[int(line[1])]] = 1

sp.sparse.save_npz('../grand_blend/adj_matrix_grand.npz', sp.sparse.csr_matrix(np_adj_matrix))
...
print("Number of instances in training (0), test (1) and validation (2)",Counter(distribution_samples),sep='\n')
...
np.save('../grand_blend/y_mat_grand.npy', y_mat)
...
json.dump({str(v): int(k) for k, v in id_map.items()}, fp=open("../data/hs_eppugnn-id_map_inv.json", "w+"))
