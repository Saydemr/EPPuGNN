        ...
        do
            ...
            do
                ...
                do
                    ...
                    do
                        FILE="${PWD}/emb/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb"
                        if [ -f $FILE ]; then
                            echo "$FILE exists"
                            ...
                            do
                                ...
                                if [[ $option -eq 1 ]]
                                then
                                    emb_filename="csv_imp/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb.csv"
                                    es_filename="csv_imp/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb_out.csv"
                                    python3 boost.py $emb_filename $es_filename $option >> $log_file                                
                                elif [[ $option -eq 2 ]]
                                then
                                    emb_filename="csv_imp_sl/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb.csv"
                                    es_filename="csv_imp_sl/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb_out.csv"
                                    python3 boost.py $emb_filename $es_filename $option >> $log_file
                                elif [[ $option -eq 3 ]]
                                then
                                    emb_filename="csv_imp_ge/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb.csv"
                                    es_filename="csv_imp_ge/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb_out.csv"
                                    python3 boost.py $emb_filename $es_filename $option >> $log_file
                                else
                                    emb_filename="csv_imp_sl_ge/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb.csv"
                                    es_filename="csv_imp_sl_ge/hs_ppi_emb_d${dim}_e${epochs}_l${length}_w${walk}_k${cont}_p${retpar}.emb_out.csv"
                                    python3 boost.py $emb_filename $es_filename $option >> $log_file
                               fi
                           done
                        else
